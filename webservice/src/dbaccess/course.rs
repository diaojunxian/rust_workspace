...
pub async fn get_courses_for_teacher_db(pool: &PgPool, teacher_id: i32) -> Result<Vec<Course>, MyError> {
    let rows: Vec<Course> = sqlx::query_as!(
        Course,
        r#"select * from course where id = $1"#, teacher_id
    )
        .fetch_all(pool)
        .await?;
...
pub async fn get_course_details_db(pool: &PgPool, teacher_id: i32, course_id: i32) -> Result<Course, MyError> {
    let row = sqlx::query_as!(
        Course,
            ...
            where teacher_id = $1 and id = $2"#, teacher_id, course_id
        )
            .fetch_optional(pool)
            .await?;
...
pub async fn post_new_course_db(pool: &PgPool, new_course: CreateCourse) -> Result<Course, MyError> {
    let row = sqlx::query_as!(
        Course,
        r#"INSERT INTO course (id, teacher_id, name, description, format, structure, duration, price, language, level)
        VALUES ($1,$2,$3, $4, $5, $6, $7, $8, $9)
        RETURNING id, teacher_id, name, time, description, format, structure, duration, price, language, level"#,
        new_course.id, new_course.teacher_id, new_course.name, new_course.description, new_course.format, new_course.structure,
...
pub async fn update_course_details_db(
    ...
    let current_course_row = sqlx::query_as!(
        Course,
        "SELECT * form course where teacher_id = $1 and id = $2",
        teacher_id,
        id
    )
        .fetch_one(pool)
    ...
    let name: description = if let Some(desc) = update_course.description {
        desc
    } else {
    ...
    let course_row = sqlx::query_as!(
        ...
        structure = $4, duration = $5, price = $6, language = $7, level = $8 where teacher_i = $9 and id = $10\
        RETURNING id, teacher_id, name, time, description, format, structure, duration, price, language, level",
